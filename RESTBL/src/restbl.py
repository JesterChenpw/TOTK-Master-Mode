...
def MergeMods(mod_path, romfs_path, restbl_path='', version=121, compressed=True, delete=False, smart_analysis=True, checksum=False):
    ...
    restbl.Reserialize(output_dir = f'Mods/{version}/Master Mode Core/romfs/System/Resource')
    if compressed:
        with open(f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename, 'rb') as file:
            data = file.read()
        if os.path.exists(f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename + '.zs'):
            os.remove(f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename + '.zs')
        os.rename(f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename, f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename + '.zs')
        with open(f'Mods/{version}/Master Mode Core/romfs/System/Resource/' + restbl.filename + '.zs', 'wb') as file:
