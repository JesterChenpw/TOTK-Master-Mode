from RESTBL.src.utils import *
import RESTBL.src.zstd as zstd
try:
    import zstandard as zs
    import yaml
except ImportError:
    raise ImportError("Would you be so kind as to LEARN TO FUCKING READ INSTRUCTIONS")
import RESTBL.src.sarc as sarc
import os
import binascii
import json
import sys
...
class Restbl:
    ...
    def _GenerateHashmap(self, paths=[]):
        if paths == []:
            ...
            string_list = "RESTBL/string_lists/" + version.replace('.', '') + ".txt"
    ...
    def GenerateChangelog(self):
        original_filepath = "RESTBL/restbl/ResourceSizeTable.Product." + self.game_version + ".rsizetable.json"
    ...
    def GenerateChangelogFromMod(self, mod_path, dump_path='', checksum=False):
        ...
        with open(get_correct_path('RESTBL/restbl/ResourceSizeTable.Product.' + self.game_version + '.rsizetable.json'), 'r') as f:
...
def GetInfoWithChecksum(romfs_path, dump_path='', version=121):
    ...
    if dump_path == '':
        dump_path = romfs_path
    zs = zstd.Zstd(dump_path)
    with open(get_correct_path('RESTBL/checksums/TearsOfTheKingdom' + str(version).replace('.', '') + '.json'), 'r') as f:
...
def MergeMods(mod_path, romfs_path, restbl_path='', version=121, compressed=True, delete=False, smart_analysis=True, checksum=False):
    ...
    restbl.Reserialize(output_dir = f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource')
    if compressed:
        with open(f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename, 'rb') as file:
            data = file.read()
        if os.path.exists(f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename + '.zs'):
            os.remove(f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename + '.zs')
        os.rename(f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename, f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename + '.zs')
        with open(f'Mods/{version}/Master Mode Core + Gameplay changes/romfs/System/Resource/' + restbl.filename + '.zs', 'wb') as file:
...
def open_tool():
    ...
    if event == 'Generate RESTBL from Mod(s)':
    ...
    elif event == 'Select RESTBL to Merge':
...
def gen_changelog():
    ...
    if event == 'JSON Changelog':
        changelog = restbl.GenerateChangelog()
        with open('changelog.json', 'w') as f:
            json.dump(changelog, f, indent=4)
    elif event == 'RCL':
        restbl.GenerateRcl()
    elif event == 'YAML Patch':
...
def apply_patches(compressed=True):
    ...
    changelogs = []
    for patch in patches:
        if os.path.splitext(patch)[1].lower() == '.json':
        ...
        elif os.path.splitext(patch)[1].lower() in ['.yaml', '.yml']:
            changelogs.append(restbl.GenerateChangelogFromYaml(os.path.join(patches_path, patch)))
        elif os.path.splitext(patch)[1].lower() == '.rcl':
